...
libAuthorized :: Key MadLib -> Handler AuthResult
libAuthorized libId = do
    mauth <- maybeAuth
    case mauth of
        ...
        Just (Entity uId user)
            ...
            | otherwise      -> do
                authorized <- runDB $ do
                    lib <- get404 libId
                    return $ uId == madLibUser lib
                if authorized then return Authorized else unauthorizedI MsgNoPermission
...
instance RenderMessage App FormMessage where
    ...
    renderMessage m (_:xs) = renderMessage m xs
